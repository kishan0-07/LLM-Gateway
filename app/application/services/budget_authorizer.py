...
class BudgetAuthorizer:
    ...
    async def authorize(
        ...
        output_tokens = self._token_estimator.estimate_max_output_tokens(
            model, requested_max_tokens
        )
        estimated_cost_usd = model_catalog.estimate_cost_usd(
        ...
        *,
        ...
        output_tokens = len(
            self._token_estimator._get_encoder(
                model_catalog.get(model).tokenizer_hint
            ).encode(accumulated_text)
        )
        provisional_cost_usd = Decimal(
            str(model_catalog.estimate_cost_usd(model, input_tokens, output_tokens))
        )

        approved_estimated_cost_usd = (
            await self._budget_store.reservation_estimated_cost_usd(reservation_id)
        )
    ...
    async def settle(
        ...
        actual_cost_usd = model_catalog.estimate_cost_usd(
            model, input_tokens, output_tokens
        )
    ...
    async def mark_needs_reconciliation(
        ...
        """Public wrapper — use cases must not access _budget_store directly."""
