...
async def main():
    ...
    await cb.record_failure(provider, model)
    assert await cb.is_available(provider, model), "2 failures < threshold=3, still CLOSED"
    print(" 2 failures, still CLOSED")

    await cb.record_failure(provider, model)
    assert not await cb.is_available(provider, model), "3 failures = threshold, should be OPEN"
    print(" 3 failures, circuit OPEN")

    print("   Waiting 6 seconds for recovery timeout...")
    await asyncio.sleep(6)
    assert await cb.is_available(provider, model), "after recovery_timeout, should be HALF_OPEN (available)"
    ...
    assert failures == "0" or failures is None, f"failures should be reset, got {failures}"
    ...
    await cb.record_failure(provider, model)  
    assert not await cb.is_available(provider, model), "failure in HALF_OPEN re-opens circuit"
