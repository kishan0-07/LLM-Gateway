...
async def test_unknown_model_rejects_before_request_creation():
    ...
    with pytest.raises(ProviderError) as exc_info:
        await use_case.execute(CompletionRequest(
            tenant_id=1, api_key_id=10, trace_id="trace-1", model="invalid/model", messages=[]
        ))
...
async def test_budget_rejection_never_calls_provider():
    ...
    use_case = build_use_case(budget=budget, routing=StaticRouting([]), circuit=RecordingCircuit(), events=CapturingEventSink())
    
    with pytest.raises(ProviderError) as exc_info:
        await use_case.execute(CompletionRequest(
            tenant_id=1, api_key_id=10, trace_id="trace-1", model="openai/gpt-oss-20b", messages=[]
        ))
...
async def test_retryable_provider_failure_falls_back_and_settles_success():
    first = ScriptedProvider("groq", [ProviderError("groq", "timeout", "forced timeout", True)])
    second = ScriptedProvider("openai", [
        ProviderResult("openai", "gpt-5.4-mini", "fallback answer", 12, 8, "actual", 7)
    ])
    routing = StaticRouting([
    ...
    circuit = RecordingCircuit()
    events = CapturingEventSink()
    use_case = build_use_case(budget=budget, routing=routing, circuit=circuit, events=events)

    response = await use_case.execute(CompletionRequest(
        tenant_id=1, api_key_id=10, trace_id="unit-fallback", model="openai/gpt-oss-20b", messages=[{"role": "user", "content": "hello"}]
    ))
    ...
    assert [call.kwargs["status"] for call in use_case._finish_provider_attempt.await_args_list] == ["timeout", "success"]

@pytest.mark.asyncio
async def test_invalid_output_falls_back_without_circuit_failure():
    first = ScriptedProvider("groq", [
        ProviderResult("groq", "openai/gpt-oss-20b", "   ", 10, 0, "actual", 5)
    ])
    second = ScriptedProvider("openai", [
        ProviderResult("openai", "gpt-5.4-mini", "valid fallback", 10, 5, "actual", 6)
    ])
    routing = StaticRouting([
        RouteCandidate(first, "openai/gpt-oss-20b", 0),
        RouteCandidate(second, "gpt-5.4-mini", 1),
    ...
    budget = RecordingBudgetAuthorizer(ReservationResult(True, "reservation-1"))
    circuit = RecordingCircuit()
    use_case = build_use_case(budget=budget, routing=routing, circuit=circuit, events=CapturingEventSink())

    response = await use_case.execute(CompletionRequest(
        tenant_id=1, api_key_id=10, trace_id="unit-invalid", model="openai/gpt-oss-20b", messages=[{"role": "user", "content": "hello"}]
    ))

    assert response.provider == "openai"
    assert circuit.failures == []  # No circuit failure!
    assert circuit.successes == [("openai", "gpt-5.4-mini")]
    assert [call.kwargs["status"] for call in use_case._finish_provider_attempt.await_args_list] == ["invalid_output", "success"]

@pytest.mark.asyncio
async def test_all_candidates_failed_settles_error_and_marks_request_failed():
    first = ScriptedProvider("groq", [ProviderError("groq", "server_error", "fail", True)])
    routing = StaticRouting([RouteCandidate(first, "openai/gpt-oss-20b", 0)])
    budget = RecordingBudgetAuthorizer(ReservationResult(True, "reservation-1"))
    events = CapturingEventSink()
    use_case = build_use_case(budget=budget, routing=routing, circuit=RecordingCircuit(), events=events)

    with pytest.raises(AllProvidersFailedError):
        await use_case.execute(CompletionRequest(
            tenant_id=1, api_key_id=10, trace_id="unit-all-fail", model="openai/gpt-oss-20b", messages=[]
        ))
...
async def test_completion_event_sanitizes_prompt_and_response():
    provider = ScriptedProvider("openai", [
        ProviderResult("openai", "gpt-5.4-mini", "Contact jane@example.com at 555-123-4567; SSN 123-45-6789", 10, 10, "actual", 5)
    ])
    routing = StaticRouting([RouteCandidate(provider, "gpt-5.4-mini", 0)])
    events = CapturingEventSink()
    use_case = build_use_case(budget=RecordingBudgetAuthorizer(ReservationResult(True, "res")), routing=routing, circuit=RecordingCircuit(), events=events)

    await use_case.execute(CompletionRequest(
        tenant_id=1, api_key_id=10, trace_id="unit-pii", model="gpt-5.4-mini", 
        messages=[{"role": "user", "content": "Email jane@example.com, phone 555-123-4567, SSN 123-45-6789"}]
    ))
